...
class user_model extends CI_Model{
    ...
    public function add($base_data, &$app_data, $old_user = FALSE){
        ...
        $query = $this->db->get(self::TB_GU);
        if($query->num_rows() == 0){
            $this->db->set('ctime', 'NOW()', FALSE);
            $this->db->replace(self::TB_GU, $app_data);
        }else{
            return FALSE;
        }
    ...
    public function no_approve($mt4_id, $user_id){
        $sql = "SELECT * FROM `user` as u "
                ...
                . "left join ".self::TB_GU." gu ON u.user_id = gu.user_id "
                . "WHERE u.user_id = '".$user_id."' AND gu.approve = 0 AND gu.mt4_id = '".$mt4_id."'";
    ...
    public function detail($user_id){   
        $sql = "SELECT * FROM `user` as u "
                . "left join `user_detail` d ON u.user_id = d.user_id "
                . "left join ".self::TB_GU." m ON u.user_id = m.user_id "
    ...
    public function list_mt4_id($user_id){
        $sql = "SELECT mt4_id FROM `".self::TB_GU."` "
    ...
    public function list_mt4($user_id, $only_approve = FALSE){
        ...
        $sql = "SELECT mt4_id, rate, approve, DATE(approve_time) approve_time, note, balance, ctime FROM `".self::TB_GU."` "
                . "WHERE user_id = '".$user_id."' ".$sql_approve." AND expired = 0 ORDER BY ctime ASC";
    ...
    public function update_balance($list_balance){       
        ...
        $sql = "UPDATE ".self::TB_GU." SET balance = CASE ".implode(' ', $sql_batch)." END WHERE mt4_id IN (".  implode(',', $list_mt4_id).")";
