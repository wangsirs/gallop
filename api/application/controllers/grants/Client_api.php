...
class client_api extends REST_Controller {
     ...
     * first_deposit,passport_path,bank_path,resident_path,rate,mod_user)
     */
    public function add_post(){
        ...
        $app_data = array();
        //必要
        foreach(array('rate', 'mod_user') as $field){
        ...
        if(isset($post['user_id']) && ! empty($post['user_id'])){
            ...
            if(empty($info)){
                $this->set_response('User not exist:'.$post['user_id'], 203);
                return FALSE;
            }
...
//        echo '</pre>';
        ...
        //審核必填欄位
        $require_field = array('first_name', 'last_name', 'pwd', 'birthday', 'email', 'country', 'city', 'address', 'cell_phone', 'zip', 'mt4_id', 'rate');
        foreach($require_field as $field){
            if( ! isset($info[$field]) || empty($info[$field])){
                $this->set_response('Grants require parameter is empty:'.$field, 203);
                return FALSE;
            }
        }
        
        //產生唯讀密碼 (pwd + 生日4碼)
        $read_pw = $info['pwd'].date('md', strtotime($info['birthday']));
        //產生交易密碼(不可給客戶)
        $main_pw = 'Dontgivetouser';
        ...
        $mt4_data = array(
			...
			'read_password' => $read_pw,  //要存在大小寫
			'email' => $info['email'],
			'country' => $info['country'],
			'state' => $info['city'],
			'city' => $info['city'],
			'address' => $info['address'],
			'comment' => empty($info['note']) ? 'grants mt4' : $info['note'],
			'phone' => $info['cell_phone'],
			'phone_password' => $main_pw.'_p',
			'status' => 'RE',
			'zipcode' => $info['zip'],
			'id' => $info['mt4_id'],
			'leverage' => '1',
