...
class Minify
{
	...
	public function __construct($config = array())
	{
        ...
        $this->_debug = $this->ci->session->userdata('minify_debug');
        $this->compress = $this->_debug ? FALSE : TRUE;
		...
		$this->scan_freq          = $this->ci->config->item('scan_freq', 'minify') ?: $this->scan_freq;

        if($this->_debug) $this->scan_freq = 0;
	 ...
	 *
	 * @param mixed $css   File or files names
     * @param bool  $prev  add to first place
	 * @param bool  $group Set group for files
	 *
	 * @return void
	 */
	public function add_css($css, $prev = FALSE, $group = 'default')
	{
		...
		{
            if($prev){
                $this->css_array[$group] = array_unique(array_merge($css, $this->css_array[$group]));
            }else{
		...
		{
            if($prev){
                $this->css_array[$group] = array_unique(array_merge(array_map('trim', explode(',', $css)), $this->css_array[$group]));
            }else{
	 ...
	 * @return void
	 */
	public function add_js($js, $prev = FALSE, $group = 'default')
	{
		...
		{
            if($prev){
                $this->js_array[$group] = array_unique(array_merge($js, $this->js_array[$group]));
            }else{
		...
		{
			if($prev){
                $this->js_array[$group] = array_unique(array_merge(array_map('trim', explode(',', $js)), $this->js_array[$group]));
            }else{
	...
	private function _deploy_css($force = TRUE, $file_name = NULL, $group = NULL)
	{
        ...
        if($this->scan_freq > 0)
        {
            $list_scan = @json_decode(@file_get_contents($this->assets_dir.'/__scan.json'), TRUE);
            if(!isset($list_scan['t']) || time() > intval($list_scan['t']) + $this->scan_freq)
            {
                $list_scan = array('t' => time(), 'jsf' => array(), 'cssf' => array());
            }
            if(empty($list_scan['cssf']) || ! in_array(trim($file_name).trim($group), $list_scan['cssf']))
            {
                $this->_scan_files('css', $force, $group);
                $list_scan['cssf'][] = trim($file_name).trim($group);
                file_put_contents($this->assets_dir.'/__scan.json', json_encode($list_scan));
            }
        }else{
            if(file_exists($this->assets_dir.'/__scan.json')){
                unlink($this->assets_dir.'/__scan.json');
            }
            $this->_scan_files('css', $force, $group);
        }
	...
	private function _deploy_js($force = FALSE, $file_name = NULL, $group = NULL)
	{
        ...
        if($this->scan_freq > 0)
        {
            ...
            if(empty($list_scan['jsf']) || ! in_array(trim($file_name).trim($group), $list_scan['jsf']))
            {
                $this->_scan_files('js', $force, $group);
                $list_scan['jsf'][] = trim($file_name).trim($group);
                file_put_contents($this->assets_dir.'/__scan.json', json_encode($list_scan));
            }
        }else{
            if(file_exists($this->assets_dir.'/__scan.json')){
	...
	private function _scan_files($type, $force, $group)
	{
		...
		if (is_array($files_array))
		{
            ...
            $line_fname = @file_get_contents($out_file.'.json');
            if(empty($line_fname) || $line_fname !== implode('|', $files_array)){
                $force = TRUE;
            }
	...
	private function _concat_files($file_array, $directory, $out_file)
	{
		...
		{
            if($this->_debug){
                fwrite($fh, '/* Minify list ===================================='.PHP_EOL.
                            '  '.implode(PHP_EOL.'  ', $file_array).PHP_EOL.
                            ' ==================================================  */'.PHP_EOL.PHP_EOL);
            }
            ...
            file_put_contents($out_file.'.json', implode('|', $file_array));
		}
		else
		{
