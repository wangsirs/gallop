    ...
    */
    ...
    public function index()
    {
        ...
        $list_btn = array(
            ...
            //array('name' => '利息明細', 'url' => $this->_app.'/trade_history', 'icon' => 'tradeOrder.png')
            );
    ...
    public function funding()
    {
        ...
        $api_re = $this->api_lib->call_api(API_PATH.'/grants/client_api/mt4s', json_encode($param));
        $data = array(
            'mt4s' => ! empty($api_re['data'])? $api_re['data'] : array()
            );
        load_frame($this->_app, __CLASS__, __FUNCTION__, $data);
    }
    ...
    public function change_pw(){
        ...
        $api_re = $this->api_lib->call_api(API_PATH.'/grants/client_api/mt4s', json_encode(array('user_id' => client_lib::user_id(),'only_approve' => '1')));
        $data = array(
            'mt4s' => $api_re['data']
            );
        load_frame($this->_app, __CLASS__, __FUNCTION__, $data);
    ...
    public function change_mt4_pw(){
        ...
        $this->load->library('api_lib');
        $api_re = $this->api_lib->call_api(API_PATH.'/grants/client_api/reset_pw', json_encode($param));
        $re->status = ($api_re['status'] === TRUE)? TRUE: FALSE;
        $re->msg = ! empty($api_re['data']) ? $api_re['data'] : '';
        die(json_encode($re));
    }
